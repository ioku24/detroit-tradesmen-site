...
import { SectionHeading, RevealOnScroll } from '../shared/Components';
...
export default function Roster() {
  ...
  const renderPlayerCard = (player: typeof ROSTER[0], index: number) => (
    <RevealOnScroll key={player.id} delay={index % 4 * 100}>
      <div className="bg-[#1a2a4a] rounded-lg overflow-hidden group border border-white/5 hover:border-blue-500/50 transition-all shadow-lg relative h-full">
        {player.isCaptain && (
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        {forwards.length > 0 && (
          <div className="mb-16">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-6">
              {forwards.map((p, i) => renderPlayerCard(p, i))}
            </div>
          </div>
        )}
        ...
        {backs.length > 0 && (
          <div className="mb-20">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-6">
              {backs.map((p, i) => renderPlayerCard(p, i))}
            </div>
          </div>
        )}
